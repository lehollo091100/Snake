...
let speed = 4;
let tileCount = 10;
function handleSpeed(action) {
  if (action == 'plus' && speed < 10) {
    speed = speed + 2;
  } else if (action == 'minus' && speed > 2) {
    speed -= 2;
  }
}
...
function isGameOver() {
  ...
  else if (headX === 10) {//if snake hits right wall
    headX = 0;
  }
  else if (headY < 0) {//if snake hits wall at the top
    headY = 10 - 1;
  }
  else if (headY === 10) {//if snake hits wall at the bottom
    headY = 0;
  }
...
function clearScreen() {
  ...
  ctx.fillStyle = '#F8DAE9'// make screen black
...
function drawSnake() {
  ...
  for (let i = 0; i < snakeParts.length; i++) {
    //draw snake parts
    let part = snakeParts[i]
    ctx.fillRect(part.x * 60, part.y * 60, tileSize, tileSize)
  }
  ...
  console.log(speed);
  const image = document.getElementById("thien");
  ctx.drawImage(image, headX * 60, headY * 60, tileSize, tileSize);
  // ctx.fillStyle = "orange";
  // ctx.fillRect(headX * 50, headY * 50, tileSize, tileSize)
...
function drawApple() {
  ...
  const image1 = document.getElementById("minhthu");
  ctx.drawImage(image1, appleX * 60, appleY * 60, tileSize, tileSize);
}
